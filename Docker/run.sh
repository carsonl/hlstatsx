...
for i in ${DIRNAME}/print-instance-args_*.sh ; do
	...
	if [ "${1}" == "${INSTANCE:1}" -o "${1}" == "" -o "${1}" == "debug" ]; then
		...
		if [ "${ALLOWMANY}" != "" ]; then
			...
			if [ "${TOOMANY}" == "yes" ]; then
				echo WARN ${0}: ${NAME}${INSTANCE} although multiple is allowed, too many are running '(10)' and nothing was 'done'. >&2
				continue
			fi
		fi
		AGELIMIT=$(grep '##AgeLimit' ${DIRNAME}/print-instance-args${INSTANCE:-"_main"}.sh)
		if [ "${AGELIMIT}" != "" ] && [ "${DCID}" != "" ]; then
			AGELIMIT=$( echo "${AGELIMIT}" | sed 's/.* //g' )
			STARTTIME=$( docker inspect "${DCID}" --format '{{ .State.StartedAt }}' | sed -E 's/\.[0-9]+Z$//g' )
			EPOCHSTART=$( date -u +%s -d "${STARTTIME}" )
			EPOCHNOW=$( date -u +%s )
			EPOCHDIFF=$( expr "${EPOCHNOW}" - "${EPOCHSTART}" )
			if [ ${EPOCHDIFF} -gt ${AGELIMIT} ]; then
				echo WARN ${0}: ${NAME}${INSTANCE} has been running too long '('Allowed: ${AGELIMIT}, Actual: ${EPOCHDIFF}')', and nothing was 'done'. >&2
				continue
			else
				if [ -t 1 ]; then
					echo INFO ${0}: Running, but allowed to run for this long. >&2
				fi
				continue
			fi
		fi
		if [ "${ALLOWMANY}" == "" ] && [ "${AGELIMIT}" == "" ] && [ "${DCID}" != "" ]; then
